        ...
        <form method="POST" action="{{ route('admin.quizzes.update', [$quest, $quiz]) }}" class="space-y-8">
            ...
            <div class="space-y-3">
                ...
                <textarea name="question" rows="3" required
                          class="w-full bg-surface-container-lowest border-4 border-black p-4 text-on-surface font-body text-2xl focus:ring-0 focus:border-[#ffd700] transition-colors outline-none resize-y">{{ old('question', $quiz->question) }}</textarea>
            </div>
                    ...
                    @foreach(['a', 'b', 'c', 'd'] as $opt)
                        <div class="flex flex-col gap-1">
                            ...
                            <div class="flex items-stretch shadow-[2px_2px_0_0_#000]">
                                <div class="w-10 bg-background border-y-4 border-l-4 border-black flex items-center justify-center font-headline text-[0.75rem] text-[#ffd700] uppercase">
                                    {{ $opt }}
                                </div>
                                <input type="text" name="option_{{ $opt }}" value="{{ old('option_' . $opt, $quiz->{'option_' . $opt}) }}" required
                                       class="flex-1 bg-surface-container-lowest border-4 border-black p-3 text-on-surface font-body text-xl focus:ring-0 focus:border-[#ffd700] outline-none">
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="grid grid-cols-4 gap-4">
                    @foreach(['a', 'b', 'c', 'd'] as $opt)
                        <label class="relative cursor-pointer group">
                            <input type="radio" name="correct_answer" value="{{ $opt }}" class="peer hidden" {{ old('correct_answer', $quiz->correct_answer) === $opt ? 'checked' : '' }} required>
